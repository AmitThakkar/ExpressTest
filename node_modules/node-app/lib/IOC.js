                ...
                if (error) {
                    ...
                    controllerFileNames.forEach(function (controllerFileName) {
                        ...
                        _.each(controller.actions, function (handler, actionName) {
                            var methodSupported = controller.allowedMethods[actionName];
                            if (methodSupported && methodSupported.length > 0) {
                                controller.allowedMethods[actionName].forEach(function (methodName) {
                                    switch (actionName) {
                                        case "get" :
                                        case "delete" :
                                            application[methodName.toLowerCase()]('/' + controllerName + "/" + actionName + "/:id", handler);
                                            break;
                                        case "list":
                                            application[methodName.toLowerCase()]('/' + controllerName + "/" + actionName + "/:max/:offset", handler);
                                            break;
                                    }
                                    application[methodName.toLowerCase()]('/' + controllerName + "/" + actionName, handler);
                                });
                            } else {
                                application["all"]('/' + controllerName + "/" + actionName, handler);
                            }
                        });
