...
exports.generateAll = function () {
    ...
    fs.readdir(process.cwd() + "/schema/", function (error, schemas) {
        if (error) {
            ...
            var controllerDirective = process.cwd() + "/controller/";
            var generateControllers = function () {
                schemas.forEach(function (schemaName) {
                    ...
                    fs.writeFile(controllerDirective + domainName + "Controller.js", controller, function (error, data) {
                        console.log(error, data)
                    });
                });
            };
            fs.exists(controllerDirective, function (exists) {
                if (exists) {
                    generateControllers();
                } else {
                    fs.mkdir(controllerDirective, function (error, dir) {
                        if (error) {
                            console.log("Error Occur at the time of creating the directive(" + controllerDirective + "). Error= ", error);
                        } else {
                            generateControllers();
                        }
                    });
