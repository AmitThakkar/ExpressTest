        ...
        if (error) {
            ...
            var generateNgViews = function (ngViewDirectory, domainName) {
                ...
                _.each(require(process.cwd() + "/schema/" + domainName), function (type, field) {
                    domainFields += "<td>" + field.replace(/(.)/, function (string, group1) {
                ...
                views.forEach(function (view) {
                    fs.readFile(__dirname + "/" + view + "Content.txt", function (error, data) {
                        if (error) {
                            ...
                            fs.writeFile(ngViewDirectory + view.toLowerCase() + ".html",
                                data.toString().replace(/{{domainNameInSmallLetter}}/g, domainNameInSmallLetter)
                                    .replace(/{{domainName}}/g, domainName).replace(/{{domainFields}}/, domainFields)
