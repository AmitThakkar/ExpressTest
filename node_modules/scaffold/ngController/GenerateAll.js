...
exports.generateAll = function () {
    ...
    var ngControllerContent = require("./Content");
    ...
    fs.readdir(process.cwd() + "/schema/", function (error, schemas) {
        if (error) {
            ...
            var generateNgControllers = function (ngControllerDirectory, domainName) {
                ...
                var newDocument = "{";
                var updatedDocument = "{\n" + CONSTANTS.TAB + CONSTANTS.TAB + CONSTANTS.TAB + "_id: $scope." + domainNameInSmallLetter + "._id,";
                var schema = require(process.cwd() + "/schema/" + domainName);
                _.each(schema, function (value, key) {
                    newDocument += ("\n" + CONSTANTS.TAB + CONSTANTS.TAB + CONSTANTS.TAB + CONSTANTS.TAB + key + ": $scope." + domainNameInSmallLetter + "." + key + ",");
                    updatedDocument += ("\n" + CONSTANTS.TAB + CONSTANTS.TAB + CONSTANTS.TAB + key + ": $scope." + domainNameInSmallLetter + "." + key + ",");
                });
                newDocument += "}";
                updatedDocument += "}";
                newDocument = newDocument.replace(/,}/, "\n" + CONSTANTS.TAB + CONSTANTS.TAB + "}");
                updatedDocument = updatedDocument.replace(/,}/, "\n" + CONSTANTS.TAB + CONSTANTS.TAB + "}");
                var controller = ngControllerContent.getText().replace(/{{domainName}}/g, domainName).
                ...
                fs.writeFile(ngControllerDirectory + "controller.js", controller, function (error, data) {
