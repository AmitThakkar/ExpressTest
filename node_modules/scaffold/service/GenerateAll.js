...
exports.generateAll = function () {
    ...
    fs.readdir(process.cwd() + "/schema/", function (error, schemas) {
        if (error) {
            ...
            var serviceDirective = process.cwd() + "/service/";
            var generateServices = function () {
                schemas.forEach(function (schemaName) {
                    ...
                    fs.writeFile(serviceDirective + domainName + "Service.js", service, function (error, data) {
                        console.log(error, data)
                    });
                });
            };
            fs.exists(serviceDirective, function (exists) {
                if (exists) {
                    generateServices();
                } else {
                    fs.mkdir(serviceDirective, function (error, dir) {
                        if (error) {
                            console.log("Error Occur at the time of creating the directive(" + serviceDirective + "). Error= ", error);
                        } else {
                            generateServices();
                        }
                    });
